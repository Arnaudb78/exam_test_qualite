...
@pytest.mark.parametrize("a, b, expected", [(1,2,3),(-1,-2,-3),(10.5,10.5,21),('chat','chien',None)])
...
@pytest.mark.parametrize("a, b, expected", [(6,2,3),(-40,-2,20),(10.5,2.5,4.2)])
def test_division_with_parametrize(a,b, expected):
    x = MathOperations()
    assert x.division(a,b) == expected

@pytest.mark.parametrize("a, b, expected", [(2,1,2),(-5,3,-125),(2,0,1), (9,0.5,3)])
